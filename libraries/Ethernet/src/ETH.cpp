...
ETHClass::ETHClass() : initialized(false), staticIP(false)
#if ESP_IDF_VERSION_MAJOR > 3
                       ,
                       eth_handle(NULL)
#endif
...
void ETHClass::disconnect()
{
    ETH.initialized = false;
    esp_eth_deinit();
    esp_eth_disable();
}
...
bool ETHClass::begin(const uint8_t mac_eth[6], uint8_t phy_addr, int power, int mdc, int mdio, eth_phy_type_t type, eth_clock_mode_t clock_mode)
{
#if ESP_IDF_VERSION_MAJOR > 3
